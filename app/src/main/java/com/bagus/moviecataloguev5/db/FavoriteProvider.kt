...
import com.bagus.moviecataloguev5.db.DatabaseContract.FavoriteColumns.Companion.AUTHORITY
...
class FavoriteProvider : ContentProvider() {
    ...
    override fun onCreate(): Boolean {
        ...
        favoriteHelper.open()
        rom = Room.databaseBuilder(context as Context, AppDatabase::class.java,"favorites").allowMainThreadQueries().build()
        return true
    }
    ...
    override fun insert(uri: Uri, contentValues: ContentValues?): Uri? {
        val fd : FavoriteDAO = rom?.favoriteDao()!!
        var added = 0
        when (FAVORITE) {
    ...
    override fun delete(uri: Uri, s: String?, strings: Array<String>?): Int {
        val deleted: Int = when (FAVORITE_ID) {
            sUriMatcher.match(uri) -> uri.lastPathSegment?.let { favoriteHelper.deleteById(it) }!!
            else -> 0
        }
        context?.contentResolver?.notifyChange(CONTENT_URI, null)
        return deleted
